...
switch (currentChargeState)
{
	...
	case ChargeState.Charging:
		if (isShooting)
		{
			var _chargeStep = set_step(chargeTime);
			
			if (chargeProgress <= 1)
			{
				chargeProgress = approach_timefactor(chargeProgress, 1, _chargeStep);
	...
	case ChargeState.Wait:
		...
		{
			var _chargeWaitStep = set_step(chargeWaitTime);
			
			if (chargeWaitProgress < 1)
			{
				chargeWaitProgress = approach_timefactor(chargeWaitProgress, 1, _chargeWaitStep);
			}
			else
			{
				currentChargeState = ChargeState.Execute;
